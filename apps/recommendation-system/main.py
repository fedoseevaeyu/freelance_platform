...
async def get_recommendations() -> Dict:
    ...
    for i, service in enumerate(services):
        ...
        top_job_indices = np.where(similarity_scores > 0.4)[0]  # Фильтрация по коэффициенту схожести
    ...
    for i, job in enumerate(job_posts):
        ...
        top_service_indices = np.where(similarity_scores > 0.4)[0]  # Фильтрация по коэффициенту схожести
